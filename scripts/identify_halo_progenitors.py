...
parser = argparse.ArgumentParser(prog='identify_halo_progenitors.py', description='Find progenitors of halos in caesar files')
...
parser.add_argument('--target_snap_nums', action='store', nargs='*', type=int, required=True, 
                    help='Snapshot numbers in which to find halo progenitors')
...
## Loop through source caesar files
for source_snap_num in args.source_snap_nums:
    ## Load snapshot
    source_snap_file = os.path.join(args.snap_dir, f'{args.snap_base}{source_snap_num:03}.hdf5')
    try:
    ...
    ## Load caesar file
    source_caesar_file = os.path.join(args.caesar_dir, f'{args.caesar_base}{source_snap_num:03}{args.caesar_suffix}.hdf5')
    try:
    ...
    # print(source_obj)
    # print([halo.GroupID for halo in source_obj.halos])
    # source_halo_ids = np.array([halo.GroupID for halo in source_obj.halos])
    # source_halo_index = np.nonzero(source_halo_ids==args.source_halo_ids)
    # print(f'source_halo_index: {source_halo_index}')
    ...
    for target_snap_num in args.target_snap_nums:
        ...
        for source_halo_id in args.source_halo_ids:
            ...
            target_halo_m500c = [target_halo.virial_quantities['m500c'].in_units('Msun').value for target_halo in target_halos]
            try:
                target_halo_contamination = [target_halo.contamination for target_halo in target_halos]
            except:
                target_halo_contamination = [np.nan for target_halo in target_halos]
            ...
            try:
                source_halo_contamination = source_halo.contamination
            except:
                source_halo_contamination = np.nan

            print(f'Writing to output file for source snap {source_snap_num}, target snap {target_snap_num}, source halo id {source_halo_id}:')
            with open(args.output_file, 'a') as f:                    
                ...
                output = f'{args.caesar_dir}\t{source_snap_num}\t{z_source}\t{source_halo_id}\t{source_halo.virial_quantities['m500c'].in_units('Msun').value}\t{source_halo_contamination}\t{target_snap_num}\t{z_target}'
                for n in range(len(target_halo_ids)):
                    output += f'\t{target_halo_ids[n]}\t{target_halo_m500c[n]}\t{target_halo_contamination[n]}'
                output += '\n'
                f.write(output)
